...
def verify_add_item(product_list, token, call_header, url):
    print("\nSTARTING PROGRAM: ", datetime.datetime.now())
    ...
    query = '''SELECT PRODUCT_ID FROM verify_posted WHERE PRODUCT_ID = ?'''
    ...
    for i in range(0, len(product_list)):
        ...
        if posted_result is None:
            ...
            item_dict = {'item_id': None, 'Title': None, 'Author': None, 'Units': None, 'Illustrator': None, 'Genre': None, 'Publisher': None, 'Publication Year': None, 'Price': None, 'Description': None, 'Condition': None, 'Condition Description': None, 'Book Format': None, 'Features': None, 'Language': None, 'Topic': None, 'Book Series': None, 'Book Type': None, 'Narrative Type': None, 'Edition': None, 'Manufactured': None, 'Inscibed': None, 'Intended Audience': None, 'Vintage': None, 'Signed': None, 'Pictures': None, 'Height': None, 'Width': None, 'Length': None, 'Weight': None, 'Weight Minor': None}
            name_value_list = []
            ...
            with open('verify_item_output.text', 'a') as f:
  ...
  <VerifyAddItemRequest xmlns="urn:ebay:apis:eBLBaseComponents">
    ...
    <Item>
      ...
      <ConditionDescription>{item_dict['Condition Description']}</ConditionDescription>
      <Country>US</Country>
      ...
      <ShippingPackageDetails>
        ...
        <WeightMinor unit="oz" measurementSystem="English">{item_dict['Weight Minor']}</WeightMinor>
      </ShippingPackageDetails>
      <Site>US</Site>
    </Item>
            ...
            print("/n")
            print(product_id)
            cursor.execute("SELECT PRODUCT_ID, MARKET_ID FROM verify_pending where PRODUCT_ID = ?", (product_id,))
            pending = cursor.fetchall()
            cursor.execute("INSERT OR IGNORE INTO verify_posted(PRODUCT_ID, MARKET_ID, DATE_TIME) VALUES (?, ?, datetime('now', 'localtime'))", (pending[0][0], pending[0][1],))
            conn.commit()
            cursor.execute("DELETE FROM verify_pending WHERE PRODUCT_ID = ?", (product_id,))
            conn.commit()
